...
try:
    url = "http://"+args+":7204/rest/repositories"
...
def run_sparql_for_cox (args):
    ...
    query1 = """
        ...
        SELECT ?data ?ID ?Tstage ?Treatment ?overall_survival_in_days ?HPV ?Nstage ?event_overall_survival
        WHERE { 
    ...
    endpoint = "http://"+args+":7204/repositories/" + repo
    annotationResponse = requests.post(endpoint,data="query=" + query1,
                                       headers={
    ...
    hnscc.to_csv('df_clin.csv', index = False) 
    #print(hnscc)
    #print(hnscc.iloc[:, 9:])
    #return hnscc
...
data = pd.read_csv("df_clin.csv")
data['event_overall_survival_bool'] = ""
data['event_overall_survival_bool'] = data['event_overall_survival'].astype('bool')
data["lp_clin"] = ""
lp_list=[]
...
data['lp_clin']=lp_list
data.drop(data.columns.difference(['ID','overall_survival_in_days', 'event_overall_survival', 'lp_clin']), 1, inplace=True)
data.to_csv('data.csv', index = False)


def run_sparql_for_cox2 (args):
    ...
    endpoint = "http://"+args+":7204/repositories/" + repo
    annotationResponse = requests.post(endpoint,data="query=" + query2,
                                       headers={
                                           "Content-Type": "application/x-www-form-urlencoded",
                                           # "Accept": "application/json"
                                       })
...
run_sparql_for_cox2("localhost")

data1 = pd.read_csv("df.csv")
data1['event_overall_survival_bool'] = ""
data1['event_overall_survival_bool'] = data1['event_overall_survival'].astype('bool')
data1["lp"] = ""
lp_list=[]
...
data1.drop(data1.columns.difference(['ID','overall_survival_in_days', 'event_overall_survival', 'lp']), 1, inplace=True)
data_rad = pd.read_csv("data.csv")
data_rad["lp"] = ""
data_rad["lp"] = data1["lp"]
data_rad.drop(data_rad.columns.difference(['overall_survival_in_days', 'event_overall_survival', 'lp_clin', 'lp']), 1, inplace=True)
data_rad['lp'].replace('', np.nan, inplace=True)
data_rad.dropna(subset=['lp'], inplace=True)
data_rad.to_csv('data_new_HN3.csv', index = False)

#cindex = result[0]
            
with open('output.txt', 'w') as f:
    #f.write(json.dumps({'Concordance Index':cindex}))
    f.write(data_rad.to_csv())